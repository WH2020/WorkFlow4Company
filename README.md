...
- 公司级统一工作台：首页、任务中心、审批中心、项目空间、公司资料库、业务域、审计和设置。
- 公司级默认 Profile `company-manager`，不默认启用任何业务域；`company-with-sales` 仅用于销售域组合验证。
- 插件清单、语义版本依赖、显式工具读写效果、权限收敛、DAG 无环/入口/出口校验。
- 统一 SQLite 任务、审批和审计运行时；结构化写入必须具有唯一直接审批前驱，进程重启会幂等恢复已提交的运行中任务。
- 任务与审批绑定公司、业务域、项目、插件版本、工作流规范指纹、策略、任务版本、载荷 SHA-256 和存储 binding；升级后不复用旧批准。
- 八个公司级共享能力：公司资料库已提供可运行的本地导入、不可变版本、检索和归档链路；模型接入、聚合搜索、知识库、项目空间、文件处理、演示文稿和定时任务保留共享契约。
- 销售域插件 `domain.sales` 及首个可验证流程 `domain.sales.pipeline-review`。
- Pi 扩展入口与公司/业务域 Skill；业务域 Skill 由启用域的清单通用贡献，启动时隔离用户全局资源，只按 Profile 显式加载项目资源，并将 Agent 工具锁定为四项只读工具（含受限资料证据检索）；RPC 测试验证实际接入。
...
- Python 3.11+
- Node.js 24.19.0 或更高的 24.x 版本、pnpm
...
启动桌面应用后进入“公司资料库”，可以直接导入 BP、专利、开发文档或公司通用资料。支持 PDF、DOCX、PPTX、XLSX、Markdown、TXT、CSV 和 JSON，单文件上限 50 MB。

- BP 默认“保密”，专利默认“高度保密”，开发文档默认“内部”。
- 上传新版本会保留旧版；历史版本切换、归档和恢复需要本人二次确认。
- 搜索在本机完成并返回原文件、版本、正文片段和内容哈希。
- 原文件和数据库位于被 Git 忽略的 `runtime/`，不会自动发送给模型或公开搜索服务。
...
- 资料库读写接口都校验本机工作台会话；文件校验扩展名、内容特征、Office 容器边界和大小，下载前复核 SHA-256。
...
- [公司资料库模块设计与实现边界](docs/06-company-library.md)
- [安装说明](docs/INSTALLATION.md)
- [开发说明](docs/DEVELOPMENT.md)

## 独立 Git 与远端

本目录是全新的独立 Git 仓库，不继承源项目历史、远端、分支或 worktree。用户已明确授权将新项目绑定到以下公司平台仓库：
...
任何新远端、可见性调整、强制推送或仓库迁移仍需再次获得用户明确授权。

## 当前限制

- 公司资料库的本地文件导入、正文抽取、不可变版本、证据检索、下载和归档已经可运行；通用项目文件协作、模型、公开搜索、知识条目写入、项目写入、演示文稿生成和定时触发仍只有插件契约或状态接入。
- 正常关窗目前通过回收子进程树保证不遗留后台服务；正式外部写入 Adapter 接入前仍需实现等待在途事务完成的优雅退出、Adapter 幂等键和崩溃恢复测试。
- 当前统一运行库用于第一阶段本机验证；生产版本仍需拆分核心库、知识库和各业务域数据库，并接入企业身份源。
- 桌面构建依赖本机 Rust、C++ Build Tools 和 WebView2；签名、安装器、自动升级和发布流程尚未配置。
- 已按用户授权绑定公司项目远端；尚未执行真实业务数据迁移。
